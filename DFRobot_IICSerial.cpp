...
DFRobot_IICSerial::DFRobot_IICSerial(TwoWire &wire, uint8_t subUartChannel, uint8_t IA1,
                                     uint8_t IA0) {
  _pWire = &wire;
...
int DFRobot_IICSerial::available(void) {
  ...
  return (index + ((unsigned int)(SERIAL_RX_BUFFER_SIZE + _rx_buffer_head - _rx_buffer_tail)) %
